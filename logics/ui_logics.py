...
def get_user_intent(text):
  intent_list = ['일상적 대화', '감상 중인 작품 밝히기', '감상 중인 그림에 대한 직접적 질문', '감상 중인 그림에 대한 간접적 질문', '추천 요청']
  ...
  if user_intent == '일상적 대화':
    ...
    system_message = '사용자가 다른 작품 또는 작가에 대해 추천해달라고 요청한다. 좀 고민하는 척 하다가, 가장 최고인 것을 1개만 추천해 주어라. 무엇을 추천하는지만 말하고, 그 이유는 말하지 마라. Format: less than 3 sentences'
    is_RAG_required = True
  else:
    is_RAG_required = False
